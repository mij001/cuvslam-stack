...
#
# Phase 1: bases + pipeline-kind mutations (the 141-run accuracy set) on the BASELINE wheel
...
CFGS=$(ls configs/base/*.toml configs/generated/*.toml 2>/dev/null | grep -v "__" | sort)
...
for NAME in $QOR; do
    CFG="configs/generated/$NAME.toml"
