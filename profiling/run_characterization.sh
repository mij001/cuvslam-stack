...
WARM=200 LAUNCHES=300 SKIP_SLAM=0 TAG="char"
while [ $# -gt 0 ]; do
    case "$1" in
        --hw) HW="$2"; shift 2 ;;
...
echo "### 2/5 nsys baseline (odometry): $CONFIG"
python3 profiling/harness/profile.py --config "$CONFIG" --profiler nsys --hw "$HW" --tag "$TAG"
NSYS_DIR=$(ls -d profiling/results/*_nsys_* | tail -1)
echo "baseline nsys: $NSYS_DIR"

NSYS_SLAM_ARG=()
if [ "$SKIP_SLAM" -eq 0 ]; then
    echo "### 3/5 nsys SLAM (loop closure): $SLAM_CONFIG"
    python3 profiling/harness/profile.py --config "$SLAM_CONFIG" --profiler nsys --hw "$HW" --tag "${TAG}-slam"
...
echo "### 4/5 ncu steady-state (characterize set, warm=$WARM launches=$LAUNCHES)"
python3 profiling/harness/profile.py --config "$CONFIG" --profiler ncu --hw "$HW" \
    ...
    --timeout 5400 --tag "${TAG}-steady"
NCU_DIR=$(ls -d profiling/results/*_ncu_* | tail -1)
echo "steady-state ncu: $NCU_DIR"
