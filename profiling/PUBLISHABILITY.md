...
|---|---|---|---|
| 🟡→✅1 | **Prototype GPU, unlocked clocks** | "All numbers come from a 25 W laptop part that can't lock clocks — nothing here is a stable quantity." | **Clock-locked workstation pass RUNNING (2026-07-03).** RTX 2000 Ada: persistence on, `-lgc 1620,1620` / `-lmc 7001,7001` (root-configured), verified pinned through the ceilings run. Measured at lock: DRAM 205.0 GB/s with 0.1 GB/s min-max spread across trials (the laptop's spread was 25%), FP32 5445 GFLOP/s ±3. Full 5-repeat characterization + SLAM + matrix in flight; closes when data lands in `reports/`. |
