...
| ⚠️NEW2 | **Slice-3 locality mixed memory spaces** (found by the attribution join, 2026-07-04) | "Your reuse/footprint numbers include shared-memory tiles and local-spill windows — those aren't data locality." | Same self-correcting-method pattern as the st_track proxy row: mem_trace records all spaces; `analysis/locality.py` parsed every opcode, so Slice-3 footprints/CDFs blend on-chip (LDS/STS) and spill (LDL/STL) accesses with global traffic. The attribution join now splits spaces (st_track = 94% spill / 5% descriptor-buffer global — its "coalesced streaming" shape is spill-dominated). TODO: add the space filter to locality.py, re-derive the Slice-3 tables from the kept traces (`~/slice3/*.zst`), annotate the report; the campaign traces (`/mnt/data/attribution_out/`) support the same re-derivation at 27-sequence scale. |
