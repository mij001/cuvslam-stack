...
| 🟡→✅4 | **No reuse-distance / locality evidence** | "DAMOV's core is locality analysis; your LFMR is a one-point proxy." | **SLICE 3 UNBLOCKED 2026-07-03 night.** Workstation downgraded to driver 575.64.05 + CUDA 12.9 on a linux-lts 6.12.39 kernel (GRUB default; rollback staged); cu12 cuVSLAM wheel rebuilt from source in the repo's podman builder; NVBit 1.8 mem_trace built (container recipe) and **capability gate passes for the first time**. Smoke-validated: 489k warp-access records, per-kernel footprints match first principles (cast_image_rgb 2.15 MB = one RGB frame + float out). Overnight traces running: TUM steady window + st_track loop-closure windows (TUM + KITTI 00) → `analysis.locality` reuse-distance-vs-capacity curves. |
