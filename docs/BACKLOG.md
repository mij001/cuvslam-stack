...
| 21 | Layer-3 **kernel-arg correlation** — name the ~40% static-memory residuals (`__device__` globals, texture reads) (THESIS G5 / Path C3) | **GROUNDWORK LAID** — `analysis/residuals.py` (runs now on committed data) ranks the exact targets: `sba::build_full_system_1/2` + `lk_track` carry **83% of all unmapped traffic** (11 kernels). The join interface is pinned in its docstring. Remaining: the NVBit kernel-arg capture tool (podman build), then reuse `attribution.py`'s LiveSet | 3–5 d |
...
| 24 | **Accel-Sim NDP configs + AccelWattch** energy — substrate-side speedup/energy *deltas* (Doc1-1/6/8, PUB open-5, THESIS G1/Path B) | **GROUNDWORK LAID** — `profiling/sim/gen_ndp_config.py` (runs now) emits Accel-Sim NDP overlays (L2 /k, DRAM ×k, PiM c) + per-kernel offload manifests from the substrate verdicts + hw descriptor, for both placement scenarios (conservative offloads 16/49, moderate 26/49). Remaining (gated): Accel-Sim checkout + calibrated sm_89 base + `blocked/run_accelsim.sh` deltas + AccelWattch energy. See `profiling/sim/README.md` | weeks |
